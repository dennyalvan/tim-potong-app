...
// muncul, dikosongkan/NULL enggak) - handleStokRollPerWarna_ (dipakai testproduksi.html) DAN
// handleStokUtuh_ (dipakai Dashboard tab Stok Kain) sama-sama skip TOTAL baris stok yang
// kode_roll-nya kosong/null, bukan cuma nyembunyiin kode rollnya tapi WARNANYA JUGA gak pernah
// nyampe ke Mini App/Dashboard sama sekali. kode_roll kosong itu valid (roll belum
// dicatat/diketahui) - sekarang cuma warna kosong atau kg_sisa<=0 yang bikin baris di-skip.
//
// ============================================================
// CODE WORKER PRODUKSI ver.16
// ============================================================
// PERUBAHAN ver.16: item CUSTOM (belum terdaftar di varian) sekarang terima warna1/warna2
// EKSPLISIT (ganti kg/kodeRoll polos versi lama, lihat komentar di handleSubmitProduksi_) -
// dipakai buat potong stok akurat, bukan nebak dari namaCustom yang bebas format. Warna ke-2
// (produk kombinasi 2 warna bahan) reuse PERSIS mekanisme ref_stok yang kombinasi biasa udah
// punya. Pasangan Mini App: testproduksi.html (form Warna 1/2 + Kg 1/2 + Kode Roll 1/2 di
// varian ITEM CUSTOM). CATATAN: potong stok custom item sekarang bergantung urutan array
// `warnaUtamaPerRow` sejajar sama `hasil` dari Supabase (asumsi POST 1x array gak diacak
// urutannya) - lihat komentar di kode.
//
// ADOPSI PERUBAHAN BESAR dari thread Apps Script terpisah (lihat MD TIM QC ver.02, CODE TIM
// POTONG ver.10.js, PANDUAN ver.04.md) - Denny sudah migrasikan skema Supabase-nya duluan,
// jadi Worker ini DISESUAIKAN mengikuti skema yang SUDAH ADA (dikonfirmasi lewat query
// information_schema.columns langsung, BUKAN asumsi):
//   - tim_potong.ukuran (JSONB) DIHAPUS, diganti 10 kolom terpisah (ukuran_xs...ukuran_6xl).
//   - log_qc DIROMBAK TOTAL: dari "1 baris per ukuran" jadi "1 baris = 1x submit", ukuran_xs
//     dst kolom terpisah, kolom Reject jadi TEKS notasi ringkas ("M1, L2"), tambah kolom
//     total/harga_jait/total_bayar. Kolom "warna" SEKARANG NAMA ITEM APA ADANYA (dulu prefix
//     dipotong - itu bug/ketidaksesuaian yang dikoreksi di thread lain).
//   - Tabel baru tarif_jahit (Varian -> Harga per Pcs), dipakai isi harga_jait/total_bayar
//     otomatis tiap submit QC (kalau Variannya ketemu di tabel itu).
//   - Endpoint BARU /data/rekap-qc - histori submit QC apa adanya (1 baris = 1x submit),
//     SENGAJA gak nyertain harga_jait/total_bayar (itu info akunting).
// Helper baru: ukuranKeKolom_/kolomKeUkuran_ (terjemahin objek {XS:n,...} <-> kolom fisik),
// parseRejectNotasi_/bangunRejectNotasi_ (notasi teks reject <-> objek).
//
// PERUBAHAN ver.14: endpoint GET /data/export-akuntansi?bulan=N - data flat SEMUA laporan
// (apapun statusnya, termasuk yang sudah SELESAI) dalam N bulan terakhir, dipakai skrip Apps
// Script terpisah (CODE SYNC AKUNTANSI.js) buat nyalin ke Google Sheet cermin buat kebutuhan
// akunting. BEDA dari /data/laporan-qc yang cuma nampilin yang BELUM selesai.
//
// PERUBAHAN ver.13 (Dashboard Tahap 4): endpoint GET /data/arsip-selesai?hari=N - daftar
// laporan yang statusnya SELESAI, difilter "N hari terakhir" (default 3, sesuai screenshot
// dashboard lama "Arsip Selesai (3 hari terakhir)").
//
// PERBAIKAN BUG (ditemukan lewat Console TypeScript Cloudflare): konstanta
// DAFTAR_UKURAN_TIMPOTONG dan fungsi cariInfoQCLengkap_ ternyata KEPAKAI di beberapa tempat
// (distribusiRingkas_, ambilInfoWarnaVarianUntukLog_) tapi belum pernah didefinisikan di file
// Worker ini - cuma ada di CODE TIM POTONG.js (Apps Script). Sekarang sudah ditambahkan.
// Juga dirapikan pemanggilan Intl.DateTimeFormat biar gak muncul warning TypeScript.
//
// PERUBAHAN ver.11 (Dashboard Tahap 3): endpoint GET /data/stok-utuh - buat tab "Stok Kain" di
// dashboard. Nampilin SEMUA roll yang masih ada sisa (kg_sisa > 0), dikelompokkan per warna
// kanonik, sekalian ringkasan total kg/roll/warna.
//
// PERUBAHAN ver.10 (Dashboard Tahap 2): endpoint POST /submit-qc - proses input QC (mode
// Ringkas: 2 angka total otomatis dituang ke ukuran yang masih kosong; mode Rincian: per
// ukuran langsung). Update status tim_potong (PROSES x/y / SELESAI), catat arsip_selesai kalau
// baru jadi SELESAI, kirim/edit notifikasi Telegram ke grup QC. Butuh 1 secret BARU:
// TELEGRAM_QC_CHAT_ID (nilai sama seperti Script Property grup QC di Apps Script).
//
// PERUBAHAN ver.09 (Dashboard Tahap 1): endpoint GET /data/laporan-qc - daftar laporan TIM
// POTONG yang BELUM SELESAI, lengkap breakdown per ukuran & progress Sel/Rej/Sisa (buat
// Dashboard Stok Kain + QC, tab "Proses & QC"). BELUM ada submit QC (itu Tahap 2 terpisah).
//
// PERUBAHAN ver.08 (Tahap 4b): dukungan item KOMBINASI (2-3 warna), porting konsep "RefStok"
// dari sistem lama. Item kombinasi kirim { kombinasi: { warna:[...], kg:[...], kodeRoll:[...] } }
// (2 atau 3 elemen) - baris tim_potong dikasih nama gabungan 2 warna pertama, warna pertama
// dipotong lewat jalur normal, warna ke-2/ke-3 dipotong TERPISAH (masing-masing by nama warna
// sendiri) dan disimpan di kolom baru ref_stok (butuh SQL ver.09 dijalankan dulu).
// CATATAN: pembatasan "Ringer harus salah satu dari 8 kombinasi warna tetap" BELUM divalidasi
// di backend ini - itu lebih pas ditegakkan di Mini App (dropdown terbatas), bukan di sini.
//
// PERUBAHAN ver.07 (Tahap 5): tambah anti-duplikat (jendela 24 jam, sama seperti sistem lama,
// tapi fingerprint sekarang dihitung dari DATA TERSTRUKTUR bukan teks - karena Mini App baru
// kirim JSON, bukan teks yang di-generate lalu di-parse ulang) + notifikasi Telegram ke grup
// produksi setelah submit sukses. Butuh 1 secret BARU: TELEGRAM_GROUP_CHAT_ID (nilai SAMA
// seperti di Apps Script Script Properties).
//
// PERUBAHAN ver.06: perbaikan bug ditemukan Denny - kg yang MINUS/NOL sekarang DITOLAK
// (sebelumnya bisa lolos dan efeknya jadi "nambahin balik" kg_terpakai, bukan menguranginya -
// bug beresiko data). Kolom kg_sisa di Supabase juga perlu diperbaiki lewat SQL ver.08
// terpisah (supaya gak pernah nunjuk minus, sama seperti sistem lama).
//
// PERUBAHAN ver.05 (Tahap 4): /submit-produksi SEKARANG JUGA memotong stok kain, porting
// PERSIS dari updateStokKain() (Apps Script) - ekstraksi kata warna (buang kode gaya),
// pencocokan via sinonim warna, kode roll WAJIB exact match kalau disebutkan (TIDAK ADA
// fallback tebak-tebak kalau kode roll gak ketemu). Item yang gak ketemu stok cocok TETAP
// masuk ke tim_potong (gak diblokir), cuma dikasih tau di response sebagai peringatan -
// PERLU DICEK MANUAL kalau ada peringatan muncul, sama seperti sistem lama (baris kuning).
//
// PERUBAHAN ver.04 (Tahap 3): endpoint POST /submit-produksi - tulis item ke tabel tim_potong
// di Supabase. BELUM ada anti-duplikat (Tahap 5).
//
// PERUBAHAN ver.03 (Tahap 2): tambah 4 endpoint GET buat data referensi/dropdown Mini App,
// baca langsung dari Supabase (gantiin ?action=kategori/warnaKanonik/stokRoll/dst yang lama).
// Butuh 2 secret BARU: SUPABASE_URL, SUPABASE_SECRET_KEY (isi sama seperti yang dipakai di
// Worker migrasi - lihat instruksi setup di bawah).
//
// PERUBAHAN ver.02: tambah header CORS (Access-Control-Allow-Origin, dst) - tanpa ini,
// request dari GitHub Pages/WebView Telegram ditolak browser (error "Failed to fetch").
//
// Worker BARU (terpisah dari "migrasirelay" yang cuma buat migrasi data) - ini backend
// PRODUKSI sungguhan buat Mini App Input Produksi, jalan di Cloudflare Workers + Supabase.
//
// TAHAP 1 (file ini): skeleton router + validasi initData Telegram doang. Tahap 2-6
// (baca data referensi, submit ke tim_potong, potong stok kain, anti-duplikat + notifikasi
// Telegram, update Mini App) MENYUSUL - ditambahkan ke file ini secara bertahap, TIDAK
// sekaligus, supaya tiap bagian bisa dites dulu sebelum lanjut.
//
// CARA DEPLOY (Worker BARU, bukan reuse "migrasirelay"):
// 1. Cloudflare dashboard -> Workers & Pages -> Create -> Workers -> nama bebas, misal
//    "tim-potong-api" -> Deploy.
// 2. Edit code -> hapus semua isi default -> paste isi file ini -> Deploy.
// 3. Settings -> Variables and Secrets -> Add (semua di-"Encrypt"):
//      TELEGRAM_BOT_TOKEN = (token bot yang SAMA seperti di Apps Script Script Properties)
//      ALLOWED_USER_ID    = opsional, sama seperti di Apps Script (kosongkan kalau semua user boleh)
// 4. Catat URL Worker-nya (https://tim-potong-api.<namauser>.workers.dev) - dipakai buat testing.
// ============================================================
